...
#define u_inlet_phy 1.20e-4//inlet velocity m/s
...
int sjpoint[Num_points][2];
int sj1 ;
int si1 ;
int sjcs[M][N];
int e[5][2] = { {0,0},{1,0},{0,1},{-1,0},{0,-1} };

int generateRandomNumber(const std::vector<int>& weights);
int numbercycle2;
struct cloggedpoint    
{
    int y;
...
int direx[] = { -1, 1, 0, 0 };
int direy[] = { 0, 0, -1, 1 };
bool isLRSectionConnected(vector<vector<int> >& grid);
...
int main()
{
    ...
    while (m1 <= mmax)
    {
        ...
        if (int(m1 * Timescale2) % 7200 == 0)
        {
            permeabilityK();
        }
        if (int(m1 * Timescale2) % 72000 == 0)
        {
        ...
        int connect = isLRSectionConnected(grid);
        if (connect == 0)
        {
            output(m1, step * Timescale2 / 3600.0);
            permeabilityK();
...
void decpoints_init()////// grain position and radius
{
    ////////////grain center coordinates (grain does not overlap) Random  ///////////

    int p2;
    
    ifstream fin1("inoculation cells.dat");
    for (p2 = 1; p2 <= Num_points; p2++)
        fin1 >> sjpoint[p2][0] >> sjpoint[p2][1] ;
...
void gks_ini()  
{
  ...
  for (int p1 = 1; p1 <= Num_points; p1++)
  {
      sjp[sjpoint[p1][1]][sjpoint[p1][0]] = 1;
  } 
    ...
    for (j = jmin; j <= jmax; j++)
        for (i = imin - 1; i <= imax + 1; i++)
        {
            if (sjp[j][i] == 3)
            {
                for (ky = 0; ky < Qy; ky++)
                {
                    if (ey[ky] > 0) for (kx = 0; kx < Qx; kx++)   //bottom                
                        f_plus[j][i][kx][ky] = f_plus[j][i][re[kx]][re[ky]] + f_plus[j + 1][i][re[kx]][re[ky]] - f_plus[j + 1][i][kx][ky];//bounce back            
                    if (ey[ky] < 0) for (kx = 0; kx < Qx; kx++) //  top          
                        f_plus[j][i][kx][ky] = f_plus[j][i][re[kx]][re[ky]] + f_plus[j - 1][i][re[kx]][re[ky]] - f_plus[j - 1][i][kx][ky];//bounce back          
                    if (ex[kx] < 0) for (ky = 0; ky < Qy; ky++)//right
                        f_plus[j][i][kx][ky] = f_plus[j][i][re[kx]][re[ky]] + f_plus[j][i - 1][re[kx]][re[ky]] - f_plus[j][i - 1][kx][ky];
                    if (ex[kx] > 0) for (ky = 0; ky < Qy; ky++)//left
                        f_plus[j][i][kx][ky] = f_plus[j][i][re[kx]][re[ky]] + f_plus[j][i + 1][re[kx]][re[ky]] - f_plus[j][i + 1][kx][ky];
                }
    ...
    for (j = jmin; j <= jmax; j++)
        for (i = imin; i <= imax; i++)
        {
            if (sjp[j][i] == 1 && contin2 == 1)
            {
                ...
                if (K[j][i] > K_min)
                {
                    epsilon[j][i] = 0.734716 - 0.58384 / (1 + 3386.87217 * exp(-0.15162 * t1 / 3600));// Equivalent porosity with time 
                    if (t1 / 3600 <= 43)
                        K[j][i] = (1.0 - 13756.75342 / (1 + 5.99397E6 * exp(-0.13252 * t1 / 3600))) * K_init;
                    if (t1 / 3600 > 43)
                        K[j][i] = (0.441 - 0.441 / (1 + 3.69732E10 * exp(-0.56666 * t1 / 3600))) * K_init;
                }
                if (K[j][i] <= K_min)
                {
                    K[j][i] = K_min;
                    ...
                    while (Neisolid == 0)
                    {
                        ...
                        {
                            int aa;
                            for (int ii = 0; ii < Neidire.size(); ++ii)
                            {
                                if (Neidire[ii] == 0) aa = 0;
                                if (Neidire[ii] == 1)aa = 6;
                                if (Neidire[ii] == 2)aa = 4;
                                if (Neidire[ii] == 4)aa = 4;
                                if (Neidire[ii] == 3)aa = 2;
                                weights.push_back(aa);
                            }
                            int dirk0 = generateRandomNumber(weights);
                            j2 = j + e[Neidire[dirk0]][1];
                            i2 = i + e[Neidire[dirk0]][0];
                            if (j2 >= 0 && i2 >= 0 && j2 <= jmax && i2 <= imax)
                            {
                                sjp[j2][i2] = 2;
                                sj1 = j2;
                                si1 = i2;
            ...
            if (sjp[j][i] == 2 && contin2 == 1)
            {
                ...
                if (K[j][i] <= K_min)
                {
                    ...
                    while (Neisolid == 0)
                    {
                        ...
                        numbercycle2 = numbercycle2 + 1;
                        
                        if (numbercycle > 1000)
                        {
                            nextpoint = nextpoint - 1;
                            sj1 = points[nextpoint].y;
                            si1 = points[nextpoint].x;
                            numbercycle = 0;
                        }                      
                    }
                }
...
void dfs(vector<vector<int> >& grid, vector<vector<bool> >& visited, int x, int y) //搜索相邻连通点
{
    int m = grid.size();
    int n = grid[0].size();
...
void output_Aver_UandF()
{
    ofstream   out1("AverFandU.dat",ios::app);
    double Fsum = 0, Faver = 0, Usum = 0, Uaver = 0;
    for (int p1 = 1; p1 <= Num_points; p1++)
    {
        int iy = sjpoint[p1][1];
        int ix = sjpoint[p1][0];
        Fsum = Fsum + pow(Fx[iy][ix] * Fx[iy][ix] + Fy[iy][ix] * Fy[iy][ix], 0.5);
        Usum = Usum + pow(ux[iy][ix] * ux[iy][ix] + uy[iy][ix] * uy[iy][ix], 0.5);
    }
    Faver = Fsum / Num_points;
    Uaver = Usum / Num_points;
    out1 << step * Timescale2 / 3600.0 << "    " << Faver << "   " << Uaver << endl;
}
void output_decpoint_UandF()
{
    ofstream   out1("decpointFandU.dat",ios::app);
    for (int p1 = 1; p1 <= Num_points; p1++)
    {
        int iy = sjpoint[p1][1];
        int ix = sjpoint[p1][0];
        out1 << step * Timescale2 / 3600.0 << "   " << ix << "   " << iy << "   " << pow(Fx[iy][ix] * Fx[iy][ix] + Fy[iy][ix] * Fy[iy][ix], 0.5) << "  " << pow(ux[iy][ix] * ux[iy][ix] + uy[iy][ix] * uy[iy][ix], 0.5) << endl;
    }
}
void cloggedpoints()
{
   
    ofstream   out1("cloggedpoints .dat",ios::app);
    for (int i1 = 0; i1 < points.size(); i1++)
    {
        out1 << step * Timescale2 / 3600.0 << "  x  " << "  " << points[i1].x <<" y "<<"   "<< points[i1].y<< endl;
    }
...
void output(int n, double m)
{
    ...
    out << "Title= \"FE-Volume Brick Data\"" << endl
        << "VARIABLES=\"X\",\"Y\",\"U\",\"V\",\"con\",\"magU\",\"Poro\",\"K\",\"Fx\",\"Fy\",\"F\",\"Sjp\",\"presure\"  "
        << endl << "ZONE Nodes=" << (imax + 1) * (jmax + 1) << "," << "Elements=" << imax * jmax << "," << "DATAPACKING=point" << "," << "ZONETYPE=FEQUADRILATERAL" << endl;
    for (j = 0; j <= jmax; j++)
        for (i = 0; i <= imax; i++)
            out << xc[i] << " "
            << yc[j] << " " << ux[j][i] << " " <<
            uy[j][i] << "       " << con[j][i] << "    " << pow(ux[j][i] * ux[j][i] + uy[j][i] * uy[j][i], 0.5) << "    " << epsilon[j][i] << "    " << K[j][i] << "    " << Fx[j][i]
            << "    " << Fy[j][i] << "    " << pow(Fx[j][i] * Fx[j][i] + Fy[j][i] * Fy[j][i], 0.5) << "    " << sjp[j][i] << "    " << p[j][i] << endl;
...
void permeabilityK()
{
    ...
    pin1 = spin / jmax; pout1 = spout / jmax; uout = uout / jmax;
    double K1 = (nu * rho0 * (xc[imax] - xc[imin]) * uout) / (pin1 - pout1);
