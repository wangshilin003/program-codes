...
double u_gra[n + 2][m + 2][8];
double shear[n + 2][m + 2][8], shear_max[n + 2][m + 2], det_co[n + 2][m + 2], u_temp[n + 2][m + 2][2], l_stress[n + 2][m + 2][8];
...
void output3( int njs) //Output--------------------------------------------------------------------------------------------------------------------------Function22
{
	...
	out << "Title= \"FE-Volume Brick Data\"" << endl
		<< "VARIABLES=\"X\",\"Y\",\"U\",\"V\",\"rho\",\"magU\",\"u  m/s\"\"es\",\"concen\",\"C  mg/L\",\"Mij\",\"conshow\"    "
		<< endl << "ZONE Nodes=" << (n + 1) * (m + 1) << "," << "Elements=" << n * m << "," << "DATAPACKING=point" << "," << "ZONETYPE=FEQUADRILATERAL" << endl;//Two-dimensional with this type ZONETYPE=FEQUADRILATERAL; three-dimensional with this type ZONETYPE=FEBRICK
	for (j = 0; j <= m; j++)
		for (i = 0; i <= n; i++)
			out << double(i) << " " << double(j) << " " << u[i][j][0] << " " <<
			u[i][j][1] << "       " << rho[i][j] << "     " << magU[i][j] << "  " << magU[i][j]*velocity_scale1 << "  " << es[i][j] << "  " << con[i][j] << "  " << con[i][j] *concf_scale2<< "  " << M[i][j] << "  " << conshow[i][j] << endl;
