...
int main()
{
    ...
    while (m1 <= mmax)
    {
        ...
        while (contin2 == 0)
        {
            ...
            if (err < 5.0e-5)contin2 = 1;
        }

        step = m1;

        if (int(m1 * Timescale2) % 18000 == 0)
        {
            permeabilityK();
        }
        if (int(m1 * Timescale2) % 180000 == 0)
        {
  ...
  for(i=imin-1;i<=imax+1;i++) //bottom 
  {
    for(ky=0;ky<Qy;ky++) 
    {
      if(ey[ky]>0) for(kx=0;kx<Qx;kx++)
          f_plus[jmin - 1][i][kx][ky] = f_plus[jmax + 1][i][kx][ky];
  ...
  for(i=imin-1;i<=imax+1;i++) // top
  {
    ...
    for(ky=0;ky<Qy;ky++)
    {
      if(ey[ky]<0) for(kx=0;kx<Qx;kx++) //            
          ...
          f_plus[jmax + 1][i][kx][ky] = f_plus[jmin - 1][i][kx][ky];
    }
  }
}
///////////////////////////
    ...
    for (j = jmin; j <= jmax; j++)
        for (i = imin; i <= imax; i++)
        {
            if (sjp[j][i] == 1 && contin2 == 1)
            {
                ...
                {
                    epsilon[j][i] = 0.734716 - 0.71598 / (1 + 19544.52347 * exp(-0.0544 * t1 / 3600));// Equivalent porosity with time 
                }
                if (epsilon[j][i] <= epsilon_min)
                {
                ...
                {                 
                    if (t1 / 3600 <= 150)
                        K[j][i] = (1.0 - 4.19757 / (1 + 1006.02018 * exp(-0.03632 * t1 / 3600))) * K_init;
                    if (t1 / 3600 > 150)
                        K[j][i] = (0.96688 - 0.96688 / (1 + 6.1649E11 * exp(-0.18857 * t1 / 3600))) * K_init;
                }
                if (K[j][i] < K_min)
                {
                    K[j][i] = K_min;
            ...
            if (sjp[j][i] == 2 && contin2 == 1)
            {
                ...
                t1 = sjcs[j][i] * Timescale2;
                if (epsilon[j][i] > epsilon_min)
                    epsilon[j][i] = 0.734716 - 0.71598 / (1 + 19544.52347 * exp(-0.0544 * t1 / 3600));
                if (K[j][i] > K_min)
                {
                    if (t1 / 3600 <= 150)
                        K[j][i] = (1.0 - 4.19757 / (1 + 1006.02018 * exp(-0.03632 * t1 / 3600))) * K_init;
...
void cloggedpoints()
{
    ...
    ofstream   out1("cloggedpoints .dat ", ios::app);//(time/h- permeability/m2 )
...
void UandP()
{
    ...
    ofstream   out1("U and P ", ios::app);
    for (int j = jmin; j <= jmax; j++)
    {
        out1 << step * Timescale2 / 3600.0 <<"   j   "<<j<< " uxn " << ux[j][imax] << "  pin  " << p[j][imin] << "  pout  " << p[j][imax] << " dp  "<<pin1-pout1<<endl;
    }
}
void output(int n, double m)
{
    ...
    out << "Title= \"FE-Volume Brick Data\"" << endl
        << "VARIABLES=\"X\",\"Y\",\"U\",\"V\",\"con\",\"magU\",\"Poro\",\"Sjp\",\"presure\"  "
        << endl << "ZONE Nodes=" << (imax + 1) * (jmax + 1) << "," << "Elements=" << imax * jmax << "," << "DATAPACKING=point" << "," << "ZONETYPE=FEQUADRILATERAL" << endl;
    for (j = 0; j <= jmax; j++)
        for (i = 0; i <= imax; i++)
            out << xc[i] << " "
            << yc[j] << " " << ux[j][i] << " " <<
            uy[j][i] << "       " << con[j][i] << "    " << pow(ux[j][i] * ux[j][i] + uy[j][i] * uy[j][i], 0.5) << "    " << epsilon[j][i] << "    " << sjp[j][i] << "    " << p[j][i] << endl;
