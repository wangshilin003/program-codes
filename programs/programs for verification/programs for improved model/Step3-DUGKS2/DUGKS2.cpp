...
#define epsilon_min 0.25
#define nuliquid_phy 1.0e-6//The kinetic viscosity m2/s
#define u_inlet_phy 1.00e-4//inlet velocity m/s
    ...
    for (j = jmin; j <= jmax; j++)
        for (i = imin; i <= imax; i++)
        {
            if (sjp[j][i] == 1 && contin2 == 1)
            {
                ...
                if (K[j][i] > K_min)
                {
                    epsilon[j][i] = 0.734716 - 0.58335 / (1 + 5977.54106 * exp(-0.08095 * t1 / 3600));// Equivalent porosity with time 
                    if (t1 / 3600 <= 88)
                        K[j][i] = (1.0 - 6047.46153/ (1 + 2.59386E6 * exp(-0.06548 * t1 / 3600))) * K_init;
                    if (t1 / 3600 > 88)
                        K[j][i] = (1.57587 - 1.57591 / (1 + 3.17611E8 * exp(-0.24497 * t1 / 3600))) * K_init;
                }
                if (K[j][i] <=K_min)
                {
                    K[j][i] = K_min;
