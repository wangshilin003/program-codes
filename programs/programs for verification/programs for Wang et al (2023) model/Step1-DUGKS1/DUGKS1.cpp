...
void gboundary()  
{
 ...
 /////
  ...
  for(i=imin-1;i<=imax+1;i++) //bottom
  {
    ...
    g_plus[jmin - 1][i][2] = g_plus[jmin - 1][i][4];
  ...
  {  
    ...
    g_plus[jmax + 1][i][4] = g_plus[jmin + 1][i][2];
  }
...
{   
   ...
   int j1=25,j2=10,j3=40;// two point at y-direction in each section
   int a1;

    ofstream   out3( "relative velocity and concentration.dat",ios::app);
   for(a1=1;a1<=5;a1++)
   {
    ...
    out3 << step * Timescale << "      " << j3 << "    " << i1[a1]
        << "       " << pow(ux[j3][i1[a1]] * ux[j3][i1[a1]] + uy[j3][i1[a1]] * uy[j3][i1[a1]], 0.5) / u_inlet << "      " <<
        con[j3][i1[a1]] / con_inlet << endl;
	out3<<"----------"<<endl;
   }
}
...
void output(int n, double m)
{
    ...
    out << "Title= \"FE-Volume Brick Data\"" << endl
        << "VARIABLES=\"X\",\"Y\",\"U\",\"V\",\"con\",\"magU\",\"Poro\",\"K\"  "
        << endl << "ZONE Nodes=" << (imax + 1) * (jmax + 1) << "," << "Elements=" << imax * jmax << "," << "DATAPACKING=point" << "," << "ZONETYPE=FEQUADRILATERAL" << endl;
    for (j = 0; j <= jmax; j++)
        for (i = 0; i <= imax; i++)
            out << xc[i] << " "
            << yc[j] << " " << ux[j][i] << " " <<
            uy[j][i] << "       " << con[j][i] << "    " << pow(ux[j][i] * ux[j][i] + uy[j][i] * uy[j][i], 0.5) << "    " << epsilon[j][i] << "    " << K[j][i] << endl;
...
void output_cross_parameter()
{
    ...
    int i, j;
    double ucross[imax + 1], concross[imax + 1];
    
    for (i = imin - 1; i <= imax; i++)
    {
        double utemp = 0, contemp = 0;
        for (int j = jmin; j <= jmax; j++)
        {
            utemp = utemp + pow(ux[j][i] * ux[j][i] + uy[j][i] * uy[j][i], 0.5);
            contemp = contemp + con[j][i];
        }
        ucross[i] = utemp /jmax/u_inlet;
        concross[i] = contemp /jmax/con_inlet;
    }

    ofstream   out3("average cross relative velocity and concentration.dat",ios::app);
    for (i = imin-1; i <= imax; i++)
    {
        out3 << step * Timescale/3600 << "      " << i << "    " << ucross[i]<< "       " << concross[i] << endl;          
    }
